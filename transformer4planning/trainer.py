...
def compute_metrics_waymo(prediction: EvalPrediction):
    from transformer4planning.utils.waymo_utils import tensor_to_str, waymo_evaluation, SUBMISSION_PATH, generate_protobuf
    pred_dicts = prediction.predictions['prediction_generation']
    
    if SUBMISSION_PATH is not None:
        task = SUBMISSION_PATH.split("_")[-1]
        print("Saving submission protobuf file for task", task)
        if os.path.exists(SUBMISSION_PATH):
            print("Skip submission generation at process", os.getpid())
        else: 
            os.mkdir(SUBMISSION_PATH)
        
            pred_dict_by_scenario = {}
            for i, scene_id in enumerate(pred_dicts["scenario_id"]):
                scene_id = tensor_to_str(torch.from_numpy(scene_id).unsqueeze(0))[0]
                if scene_id not in pred_dict_by_scenario:
                    pred_dict_by_scenario[scene_id] = {}
                
                if "object_id" in pred_dict_by_scenario[scene_id] and pred_dicts["object_id"][i] in pred_dict_by_scenario[scene_id]["object_id"]:
                    continue
                
                for key in ["pred_trajs", "object_id", "pred_scores"]:
                    if key not in pred_dict_by_scenario[scene_id]:
                        pred_dict_by_scenario[scene_id][key] = []
                        
                    pred_dict_by_scenario[scene_id][key].append(pred_dicts[key][i])
            
            with open(os.path.join(SUBMISSION_PATH, "pred_dict.pkl"), "wb") as f:
                pickle.dump(pred_dict_by_scenario, f)
                
            print("Prediction dictionary is saved at", os.path.join(SUBMISSION_PATH, "pred_dict.pkl"))
            # generate_protobuf(pred_dict_by_scenario, SUBMISSION_PATH.split("_")[-1], SUBMISSION_PATH, "submission.protobuf")
...
def compute_metrics_simagents(prediction: EvalPrediction):
    ...
    pred_by_scenarios = {}
    for idx in range(len(pred_dicts["scenario_id"])):
        scene_id = tensor_to_str(torch.from_numpy(pred_dicts["scenario_id"][idx]).unsqueeze(0))[0]
        if "object_id" in pred_dicts.keys() and "pred_trajs" in pred_dicts.keys():
            ...
            file_path = os.path.join(os.environ["TMP_PRED_DIR"], scene_id + ".pkl")
            with open(file_path, "rb") as f:
                pred_by_scenarios[scene_id] = pickle.load(f)
...
class PlanningTrainer(Trainer):
    ...
    def prediction_step(
        ...
        # if logits.shape[0] != self.args.per_device_eval_batch_size and logits.shape[0] % self.args.per_device_eval_batch_size != 0:
        ...
        if self.model.config.task == "simagents":
            logits_dict = {
