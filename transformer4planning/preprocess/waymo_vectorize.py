...
from transformer4planning.utils.waymo_utils import rotate_points_along_z, merge_batch_by_padding_2nd_dim, MAX_SCENE_LENGTH
...
def scenario_collate_func(batch, dic_path=None):
    map_func = partial(scenario_preprocess, data_path=dic_path)
...
def waymo_preprocess(sample, data_path):
    ...
    split = sample["split"]
    if split == "train" and data_path == "/localdata_ssd/liderun/t4p_waymo_interaction": 
        file = os.path.join("/localdata_ssd/liderun/t4p_waymo_full", f"{split}", scene_id + ".pkl")
    else:
        file = os.path.join(data_path, f"{split}", scene_id + ".pkl")
...
def scenario_preprocess(sample, data_path):
    scene_id = sample["scenario_id"]
    track_index_to_predict = sample["track_index_to_predict"].view(-1)
    split = sample["split"]    
    ...
    sdc_track_index = info["sdc_track_index"]
    if MAX_SCENE_LENGTH is not None and len(track_index_to_predict) > MAX_SCENE_LENGTH: 
        track_index_to_predict = track_index_to_predict[:MAX_SCENE_LENGTH]
        if sdc_track_index not in track_index_to_predict: track_index_to_predict[0] = sdc_track_index
