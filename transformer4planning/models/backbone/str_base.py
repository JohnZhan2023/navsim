...
class STR(PreTrainedModel):
    ...
    def build_encoder(self):
        if self.config.task == "nuplan":
            ...
            if (self.config.task == "simagents" or self.config.task == "interaction") and self.config.decoder_type == "diffusion" and self.config.debug_scenario_encoding:
    ...
    def forward(
        ...
        attention_mask = info_dict["input_attn_mask"] if "input_attn_mask" in info_dict else None
        transformer_outputs_hidden_state = self.embedding_to_hidden(input_embeds, attention_mask=attention_mask, return_dict=return_dict)
    ...
    def generate(self, **kwargs) -> torch.FloatTensor:
        ...
        if self.config.autoregressive:
            return self.generate_autoregressive(input_embeds, info_dict, kwargs)
        ...
        if self.config.decoder_type == "diffusion":
            attention_mask = info_dict["input_attn_mask"] if "input_attn_mask" in info_dict else None
            transformer_outputs_hidden_state = self.embedding_to_hidden(input_embeds, attention_mask=attention_mask)
        ...
        if self.config.task == "waymo" or self.config.task == "interaction" or self.config.task == "simagents":
            from transformer4planning.utils.waymo_utils import trans_ego_to_world, str_to_tensor, MAX_SCENE_LENGTH
            center_objects_world = kwargs['center_objects_world'].type_as(traj_pred_logits)
            pred_dict = {}
            
            if self.config.debug_scenario_encoding:     
                assert "scene_length" in info_dict
                scenario_id_list, pred_trajs_world_list, score_list = [], [], []
                scene_start = 0
                for s, l in enumerate(info_dict["scene_length"]):
                    scene_end = scene_start + l
                    scenario_id = kwargs['scenario_id'][scene_start]
                    if self.config.debug_scene_level_prediction:
                        assert MAX_SCENE_LENGTH is not None
                        traj_length = traj_pred_logits.shape[2] // MAX_SCENE_LENGTH
                        pred_trajs_scene = traj_pred_logits[s, :, :l*traj_length, :].view(self.k, l, traj_length, -1).transpose(1, 0)
                    else:
                        pred_trajs_scene = traj_pred_logits[scene_start:scene_end]
                        
                    center_objects_world_scene = center_objects_world[scene_start:scene_end]
                    pred_trajs_world = trans_ego_to_world(pred_trajs_scene, center_objects_world_scene, self.config.predict_yaw)
                    
                    if self.config.task == "simagents":
                        file_path = os.path.join(os.environ["TMP_PRED_DIR"], scenario_id + ".pkl")
                        if os.path.exists(file_path): os.remove(file_path)
                        pkl_dict = {
                            'pred_trajs': pred_trajs_world.transpose(0, 1).cpu().numpy(),
                            'object_id': kwargs['center_objects_id'][scene_start:scene_end],
                        }
                        ...
                        pred_trajs_world_list.append(pred_trajs_world)
                        score_list.append(topk_score[s].unsqueeze(0).repeat(l, 1))
                    
                    scene_start = scene_end
                    
                if self.config.task == "simagents":     
                    pred_dict.update({
                        'scenario_id': str_to_tensor(scenario_id_list).to(device)
                    })
                else:
                    pred_dict.update({
                        'scenario_id': str_to_tensor(kwargs['scenario_id']).to(device),
                        'pred_trajs': torch.cat(pred_trajs_world_list, dim=0)[..., 0:2],
                        'object_id': torch.tensor(kwargs['center_objects_id']).to(device),
                        'pred_scores': torch.cat(score_list, dim=0),
                ...
                pred_trajs_world = trans_ego_to_world(traj_pred_logits, center_objects_world, self.config.predict_yaw)
                ...
                if self.config.task == "simagents":
                    ...
                    if self.k > 1 and not self.use_proposal and self.use_key_points == "no" and self.config.decoder_type != "diffusion":
                        topk_score = torch.ones((pred_trajs_world.shape[0], self.k), device=device) / self.k
    ...
    @torch.no_grad()
    def generate_autoregressive(self, input_embeds, info_dict, input_dict):
        ...
        pred_length = info_dict["pred_length"]
        sample_interval = 10
        pred_frames = pred_length // sample_interval
        for r in range(pred_frames-1):
            if traj_logits.shape[1] > self.config.mc_num:
                scores, topk_index = torch.topk(scores, k=self.config.mc_num, dim=-1)
                traj_logits = torch.stack([traj_logits[i, topk_index[i]] for i in range(scores.shape[0])])
            
            scores = torch.softmax(scores, dim=1)
            action_embed = self.encoder.action_m_embed(traj_logits[:, :, :(r+1)*sample_interval, :])
            ...
            for m in range(action_embed.shape[1]):
                ...
                input_embeds_m[:, -pred_length:-pred_length+(r+1)*sample_interval, :] = action_embed_m
                ...
                scores_m = torch.softmax(scores_m, dim=1)
        ...
        traj_pred_logits = torch.cat(reg_sigma_cls_dict["reg"],dim=0)
        scores = torch.cat(reg_sigma_cls_dict["cls"],dim=0)
        
        center_objects_world = input_dict['center_objects_world'].type_as(traj_pred_logits)
        ...
        if self.config.debug_scenario_encoding:
            scenario_id = input_dict['scenario_id'][0:1]
            file_path = os.path.join(os.environ["TMP_PRED_DIR"], scenario_id[0] + ".pkl")
            if os.path.exists(file_path): os.remove(file_path)
            pkl_dict = {
                'pred_trajs': pred_trajs_world.transpose(0, 1).cpu().numpy(),
                'object_id': input_dict['center_objects_id'],
            }
            with open(file_path, "wb") as f:
                pickle.dump(pkl_dict, f)
                
            pred_dict.update({
                'scenario_id': str_to_tensor(scenario_id).to(device)
            })
        else:
            pred_dict.update({
                'scenario_id': str_to_tensor(input_dict['scenario_id']).to(device),
                'pred_trajs': pred_trajs_world,
                'object_id': torch.tensor(input_dict['center_objects_id']).to(device),
            })
    ...
    if 'scratch' in model_args.model_name:
        ...
        if model_args.freeze_backbone:
            ...
            for param in model.transformer.parameters():
                param.requires_grad = False
            for param in model.encoder.relapos_embedding.parameters():
