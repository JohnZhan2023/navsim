...
from navsim.agents.transfuser_str.TranDPconfig import TranDPConfig
from navsim.agents.diffusion_policy.DPcallback import DPCallback
from navsim.agents.transfuser_str.TranDPfeatures import DPTargetBuilder,DPFeatureBuilder
from diffusion_policy.model.diffusion.conditional_unet1d import ConditionalUnet1D
from diffusion_policy.policy.diffusion_unet_image_policy import DiffusionUnetImagePolicy
from diffusion_policy.model.diffusion.mask_generator import LowdimMaskGenerator
from diffusion_policy.model.common.normalizer import LinearNormalizer
from diffusion_policy.common.pytorch_util import dict_apply
from navsim.agents.res_str.STRencoder import STREncoder
...
class TranDPagent(AbstractAgent):
    ...
    def __init__(
        ...
        self.obs_feature_dim=config.n_embd
        ...
        self.k=config.k
        self.obs_encoder=STREncoder(self.obs_feature_dim,config)
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        input_embeds, info= self.obs_encoder(features)
        return_dict = True
        input_embeds = input_embeds.view(-1, info["context_length"], self.obs_feature_dim)
    ...
    def compute_loss(
        ...
        input_embeds, info = self.obs_encoder(batch)
        return_dict = True
        input_embeds = input_embeds.view(-1, info["context_length"], self.obs_feature_dim)
        #print("when training, the input_embeds shape is",input_embeds.shape)
        padding = torch.zeros(input_embeds.size(0), 8, input_embeds.size(2), dtype=input_embeds.dtype, device=input_embeds.device)
        padded_input_embeds = torch.cat([input_embeds, padding], dim=1)
        print("the shape of padded_input_embeds is",padded_input_embeds.shape)
        transformer_outputs = self.transformer(
            inputs_embeds=padded_input_embeds,
            attention_mask=None,
            return_dict=return_dict,
        ...
        transformer_outputs_hidden_state = transformer_outputs['last_hidden_state']
        print("the shape of transformer_outputs_hidden_state is",transformer_outputs_hidden_state.shape)
        info_dict = info
