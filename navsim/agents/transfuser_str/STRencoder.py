...
class STREncoder(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        res_embed = self.cnn_downsample(lidar_feature.squeeze())
        res_embed = res_embed.view(B, T, -1)
        print("1res_embed shape",res_embed.shape)
        if self.camera_image_encoder is not None:
            ...
            camera_image = self.image_processor(camera_image, return_tensors="pt").to("cuda")
            camera_image = self.camera_image_encoder(**camera_image).last_hidden_state
            print("0camera_image shape",camera_image.shape)
            camera_image = self.camera_image_m_embed(camera_image)
            print("1camera_image shape",camera_image.shape)
            print("camera_image shape",camera_image.shape)
            res_embed = torch.cat([res_embed, camera_image], dim=1)
            print("2res_embed shape",res_embed.shape)
            context_length +=257
        ...
        action_embed = self.action_m_embed(status_feature)
        action_seq_length = T*2
        input_embeds = torch.zeros(B, action_seq_length, self.config.n_embd)
