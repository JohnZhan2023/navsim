...
from transformer4planning.models.backbone.mixtral import STRMixtralConfig, STR_Mixtral
import copy
import torch
import torch.nn as nn
...
from navsim.agents.transfuser_oneframe_str_moe.DPencoder import TranfuserEncoder
from navsim.agents.diffusion_policy.DPcallback import DPCallback
from navsim.agents.transfuser_oneframe_str_moe.TranDPfeatures import DPTargetBuilder,DPFeatureBuilder
...
class TranDPagent(AbstractAgent):
    ...
    def __init__(
        ...
        pretrainConfig = STRMixtralConfig()
        pretrainConfig.n_layer = config.n_layer
        pretrainConfig.n_embd = config.n_embd
        pretrainConfig.n_inner = config.n_inner
        pretrainConfig.n_head = config.n_head
        pretrainConfig.num_hidden_layers = config.num_hidden_layers
        pretrainConfig.hidden_size = config.hidden_size
        pretrainConfig.intermediate_size = config.intermediate_size
        pretrainConfig.num_attention_heads = config.num_attention_heads
        pretrainConfig.use_proposal = config.use_proposal
        pretrainConfig.use_key_points = config.use_key_points
        pretrainConfig.kp_decoder_type = config.kp_decoder_type
        pretrainConfig.k = config.k
        pretrainConfig.task = config.task
        ###the str###
        self.decoder=None
        self.transformer = MixtralModel(pretrainConfig)
        self.k=config.k
